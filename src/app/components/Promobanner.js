...
/*
  Komponen banner promo yang TERISOLASI.

  Kenapa dipisah dari page.js:
  - Countdown di sini ber-setState setiap 1 detik. Kalau ini berada di dalam
    komponen Home yang besar, setiap detik SELURUH halaman ikut re-render dan
    mengganggu animasi geser banner (jadi patah-patah).
  - Dengan dipisah, detak per-detik hanya me-render ulang komponen kecil ini,
    sehingga animasi banner tetap mulus.
*/
export default function PromoBanner() {
  ...
  // Ambil semua banner promo aktif (diatur admin)
  useEffect(() => {
    (async () => {
  ...
  // Countdown sampai akhir hari (hanya jalan jika ada banner dengan countdown)
    ...
    <div style={{ padding: '28px 24px 0', maxWidth: '1400px', width: '100%', margin: '0 auto' }}>
      <div style={{ position: 'relative', borderRadius: '24px', overflow: 'hidden', boxShadow: '0 18px 44px rgba(37,99,235,0.22)' }}>
        <AnimatePresence mode="wait">
          <motion.div key={b.id} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.4, ease: 'easeOut' }}
            style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: '28px', background: b.bg_color || 'linear-gradient(120deg,#2563EB 0%,#4F46E5 100%)', padding: '34px 40px', flexWrap: 'wrap', overflow: 'hidden', minHeight: '210px' }}>
        ...
        {banners.length > 1 && (
          <div style={{ position: 'absolute', bottom: '14px', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: '8px', zIndex: 3 }}>
            {banners.map((_, i) => (
              <button key={i} onClick={() => setBIdx(i)} aria-label={`Banner ${i + 1}`} style={{ width: i === bIdx ? '26px' : '9px', height: '9px', borderRadius: '999px', border: 'none', cursor: 'pointer', background: i === bIdx ? '#fff' : 'rgba(255,255,255,0.5)', transition: 'all 0.25s' }} />
            ))}
          </div>
        )}
      </div>
