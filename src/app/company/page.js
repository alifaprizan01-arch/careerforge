    ...
    <div style={{ minHeight: '100vh', background: bg, fontFamily: 'Plus Jakarta Sans, Inter, sans-serif' }}>
      ...
      <div style={{ background: card, borderBottom: `1px solid ${border}`, padding: '0 32px', display: 'flex', alignItems: 'center', height: '64px', gap: '16px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          ...
          <span style={{ fontSize: '11px', padding: '4px 10px', borderRadius: '6px', background: '#EA580C', color: '#fff', fontWeight: 700, letterSpacing: '0.04em' }}>PERUSAHAAN</span>
      ...
      <main style={{ padding: '32px', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px', marginBottom: '28px' }}>
          {[
            { label: 'Lowongan Aktif', value: stats.lowongan, icon: '💼', color: '#EA580C', bg: isDark ? 'rgba(234,88,12,0.15)' : '#FFF7ED', href: '/company/lowongan' },
            { label: 'Total Pelamar', value: stats.lamaran, icon: '👥', color: '#2563EB', bg: isDark ? 'rgba(37,99,235,0.15)' : '#EFF6FF', href: '/company/lamaran' },
            { label: 'Menunggu Review', value: stats.menunggu, icon: '⏳', color: '#D97706', bg: isDark ? 'rgba(217,119,6,0.15)' : '#FFFBEB', href: '/company/lamaran' },
            { label: 'Diterima', value: stats.diterima, icon: '✅', color: '#16A34A', bg: isDark ? 'rgba(22,163,74,0.15)' : '#F0FDF4', href: '/company/lamaran' },
          ].map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
              whileHover={{ y: -3, boxShadow: '0 8px 20px rgba(0,0,0,0.1)' }}
              onClick={() => router.push(s.href)}
              style={{ background: card, borderRadius: '12px', border: `1px solid ${border}`, borderLeft: `4px solid ${s.color}`, padding: '20px', display: 'flex', alignItems: 'center', gap: '14px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', transition: 'all 0.2s', cursor: 'pointer' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px', marginBottom: '28px' }}>
          {[
            { href: '/company/lowongan', icon: '➕', label: 'Buat Lowongan', desc: 'Tambah posisi yang dibutuhkan', color: '#EA580C', bg: isDark ? 'rgba(234,88,12,0.15)' : '#FFF7ED' },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
          <div style={{ background: card, borderRadius: '12px', border: `1px solid ${border}`, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
            <div style={{ padding: '16px 20px', borderBottom: `1px solid ${border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <Link href="/company/lamaran" style={{ fontSize: '12px', color: '#EA580C', fontWeight: 600, textDecoration: 'none' }}>Lihat semua →</Link>
            </div>
            ...
            recentApps.length === 0 ? (
              <div style={{ padding: '48px', textAlign: 'center' }}>
                ...
                <Link href="/company/lowongan" style={{ display: 'inline-block', marginTop: '12px', padding: '7px 16px', borderRadius: '8px', background: '#EA580C', color: '#fff', fontWeight: 600, fontSize: '12px', textDecoration: 'none' }}>Buat Lowongan</Link>
              </div>
            ) : recentApps.slice(0,5).map((app, i) => (
              <div key={app.id} onClick={() => router.push('/company/lamaran')} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '11px 20px', borderBottom: i < Math.min(recentApps.length,5)-1 ? `1px solid ${border}` : 'none', transition: 'background 0.15s', cursor: 'pointer' }}
                ...
                {app.users?.avatar_url ? <img src={app.users.avatar_url} style={{ width: '30px', height: '30px', borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} /> :
                  <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: 'linear-gradient(135deg,#EA580C,#C2410C)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: '10px', flexShrink: 0 }}>
                ...
                {app.status === 'Menunggu' ? (
                  <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                    <button onClick={(e) => { e.stopPropagation(); updateAppStatus(app.id, 'Diterima'); }} disabled={updatingId === app.id}
                      title="Terima" style={{ width: '26px', height: '26px', borderRadius: '7px', border: 'none', background: '#16A34A', color: '#fff', cursor: updatingId === app.id ? 'wait' : 'pointer', fontSize: '13px', fontWeight: 700 }}>✓</button>
                    <button onClick={(e) => { e.stopPropagation(); updateAppStatus(app.id, 'Ditolak'); }} disabled={updatingId === app.id}
                      title="Tolak" style={{ width: '26px', height: '26px', borderRadius: '7px', border: `1px solid ${border}`, background: subtle, color: '#DC2626', cursor: updatingId === app.id ? 'wait' : 'pointer', fontSize: '13px', fontWeight: 700 }}>✕</button>
                  </div>
                ) : (
          ...
          <div style={{ background: card, borderRadius: '12px', border: `1px solid ${border}`, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
            <div style={{ padding: '16px 20px', borderBottom: `1px solid ${border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <Link href="/company/lowongan" style={{ fontSize: '12px', color: '#EA580C', fontWeight: 600, textDecoration: 'none' }}>Kelola →</Link>
            </div>
            {loading ? <div style={{ padding: '20px' }}>{[1,2,3].map(i => <div key={i} className="skeleton" style={{ height: '52px', marginBottom: '8px' }} />)}</div> :
            myJobs.length === 0 ? (
              <div style={{ padding: '48px', textAlign: 'center' }}>
              ...
              <div key={job.id} onClick={() => router.push('/company/lowongan')} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '11px 20px', borderBottom: i < Math.min(myJobs.length,5)-1 ? `1px solid ${border}` : 'none', transition: 'background 0.15s', cursor: 'pointer' }}
                onMouseEnter={e => e.currentTarget.style.background = subtle} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                <div style={{ width: '30px', height: '30px', borderRadius: '8px', background: isDark ? 'rgba(234,88,12,0.15)' : '#FFF7ED', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '11px', color: '#EA580C', flexShrink: 0 }}>
