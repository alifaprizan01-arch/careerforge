    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#F8FAFC', fontFamily: 'Inter, sans-serif' }}>
      ...
      <main style={{ marginLeft: '220px', flex: 1, padding: '28px 32px' }}>
        <div style={{ marginBottom: '20px' }}>
          <Link href="/trayek" style={{ color: '#2563EB', textDecoration: 'none', fontSize: '13px' }}>← Kembali ke Lowongan</Link>
        </div>
                ...
                <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start', marginBottom: '20px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                      <span style={{ fontSize: '12px', padding: '4px 12px', borderRadius: '20px', background: '#EFF6FF', color: '#2563EB', border: '1px solid #E2E8F0', fontWeight: 500 }}>{job.jenis || 'Full Time'}</span>
                      {job.asal && <span style={{ fontSize: '12px', padding: '4px 12px', borderRadius: '20px', background: '#F8FAFC', color: '#475569', border: '1px solid #E2E8F0', fontWeight: 500 }}>📍 {job.asal}</span>}
                    </div>
                  </div>
                </div>
            ...
            <div style={{ position: 'sticky', top: '28px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {hasApplied && application && (
                <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #E2E8F0', padding: '20px' }}>
                  ...
                  <div style={{ fontSize: '12px', color: '#94A3B8' }}>Dikirim: {new Date(application.applied_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</div>
                </div>
              )}
              ...
              <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #E2E8F0', padding: '20px' }}>
                {!hasApplied ? (
                  <>
                    ...
                    {!showForm ? (
                      <button onClick={() => setShowForm(true)} style={{ width: '100%', padding: '11px', borderRadius: '8px', border: 'none', background: '#2563EB', color: '#fff', fontWeight: 600, fontSize: '14px', cursor: 'pointer' }}>Lamar Sekarang</button>
                    ) : (
                      <>
                        <div style={{ marginBottom: '12px' }}>
                          ...
                          <textarea value={coverLetter} onChange={e => setCoverLetter(e.target.value)} rows={6}
                            placeholder={`Perkenalkan diri kamu dan jelaskan mengapa cocok untuk posisi ${job.tujuan}...`}
                  ...
                  <>
                    ...
                    <Link href="/lamaran" style={{ display: 'block', textAlign: 'center', marginTop: '14px', color: '#2563EB', fontSize: '13px', fontWeight: 500 }}>Lihat semua lamaran →</Link>
                  </>
                )}
              </div>
              ...
              {!hasApplied && (
                <div style={{ background: '#EFF6FF', borderRadius: '12px', border: '1px solid #BFDBFE', padding: '16px' }}>
                  ...
                  {['Sebutkan nama posisi dan sumber info lowongan', 'Jelaskan pengalaman relevan yang kamu miliki', 'Tunjukkan antusiasme terhadap perusahaan', 'Akhiri dengan ajakan untuk interview'].map((tip, i) => (
